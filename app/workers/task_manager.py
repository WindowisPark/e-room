...
class TaskManager:
    ...
    def _initialize(self):
        try:
            ...
            logger.info(f"TaskManager Redis 연결 성공: {settings.REDIS_URL}")
            ...
            self.queues = {"pdf": DummyQueue(), "ai": DummyQueue(), "default": DummyQueue()}
    ...
    def enqueue_task(
            ...
            *args, 
            ...
            db: Optional[Session] = None,  # 이 db는 작업 기록에만 사용되고 큐로 전달되지 않음
        ...
        Args:
            ...
            db: DB 세션 (선택 - 작업 기록용, 큐로 전달되지 않음)
            task_type: 작업 유형 (선택 - 작업 기록용)
            document_id: 문서 ID (선택 - 작업 기록 및 작업 함수에 전달)
            kwargs: 함수 키워드 인자
        ...
        try:
            ...
            # 작업 함수에 전달할 인자들 구성
            # 중요: Session은 pickle할 수 없으므로 db는 전달하지 않음
            func_kwargs = kwargs.copy()  # 기존 kwargs 복사
            
            # document_id와 user_id는 전달 가능
            if document_id is not None:
                func_kwargs['document_id'] = document_id
            if user_id is not None:
                func_kwargs['user_id'] = user_id
            
            # 디버깅 로그 추가
            logger.info(f"작업 등록: func={func.__name__}, document_id={document_id}, kwargs={func_kwargs.keys()}")
            
            # 작업 등록 - args와 kwargs 모두 명시적으로 전달
            job = queue.enqueue(
                func,
                args=args,
                kwargs=func_kwargs,  # db가 없는 업데이트된 kwargs 사용
                job_id=job_id,
                timeout=job_timeout,
